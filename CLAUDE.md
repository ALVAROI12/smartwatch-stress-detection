...
- `docs/advisor_correction_sheet.md`: the paper's state on 2026-09-20 (superseded; current state is in `STATUS.md` and `docs/contribution_map.md`).
...
The old README headline (94.53% accuracy, 96 subjects) came from the leaky pipeline; the README was corrected on 2026-09-22 (PR #4).
...
- Advisor and committee reports, and which one is current: see the Deliverables table in `STATUS.md`. Report sources live on the local branch `jbhi-revision-with-memo` (excluded here via `.git/info/exclude`).
...
Open decisions and deliverables are tracked in `STATUS.md`.
...
- Arousal kit (2026-09-23, `scripts/arousal_kit.py`, `docs/arousal_kit.md`): an untrained index (z `hr_mean` + z `eda_tonic_mean`) ranks stress as well as the external model on all five datasets (paired CIs include 0; UBFC-Phys touches 0). Kwon's iso-HR test reproduces (0.67–0.99). At matched arousal the model scores Lego, anger, UBFC control and hyperventilation above rest (0.62–0.79) but not exercise or fear, so its non-arousal part is task engagement. Nulls can only exclude within-bin AUROC ≥ 0.60–0.64.
- Steps 3–4 (2026-09-23): `scripts/wrist_hrv_direction.py` (WESAD: wrist captures half the ECG HR rise under TSST; wrist SDNN rises +32.8 ms [9.2, 58.1] while ECG SDNN does not; RMSSD direction not significant on either) and `scripts/stressor_potency.py` (recall vs own arousal response ρ 0.56; dataset adds ΔR² 0.10 beyond response). Manuscript condensed to 6 pages.
