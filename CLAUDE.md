...
- The current advisor report is `~/Desktop/JBHI_revision_report_for_Dr_Pan_2026-09-21.pdf`, built by `docs/make_advisor_report.py` (source committed only on the local `jbhi-revision-with-memo` branch; excluded here via `.git/info/exclude`). The older `JBHI_revision_report_for_Dr_Pan.pdf` and `docs/advisor_correction_sheet.md` predate the five-dataset results.
