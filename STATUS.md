...
1. Roadmap (`docs/review_simulation/jbhi_review.md` §4) worked through on 2026-09-23 evening: REV-1 to REV-15 addressed on existing data (new analysis folders `matched_arousal_robustness/`, `budget_time_settling/`, `eda_quality_hr_arm/`; supplement `paper/supplement.tex`; checklist Table). Findings that changed the text: exercise and fear separability at matched arousal is session-scaling specific; the UBFC/hyperventilation/Lego nulls are robust across scalings, complete-case windows and EDA screening but underpowered on small pairs (positive control); position matching fails on Stress-Predict (time-only 0.88); EPM-E4 shows no temperature rise without a stressor; HR/HRV-only proxy is worse. Still open: author e-mails (L18), a manipulation-check table from self-reports, HR-only matching on WESAD (Kwon reconciliation), feature-table release decision (dataset licences). Verification re-review running.
