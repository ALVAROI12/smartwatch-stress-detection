...
**Before submission (needs the authors):** author e-mails (`main.tex` L18); Funding and Conflict-of-Interest statements (TODO comment above Data and Code Availability); PDFs of Kreibig (2010) and Dickerson & Kemeny (2004) into `sources/` so the psychophysiology paragraph can be written with verified quotes; decide whether to release the window-level feature table (dataset licences); page budget: JBHI (embs.org/jbhi, checked 2026-09-23) allows 14 pages for regular papers including supplementary material, charges $250/page for pages 9-10 and $350/page beyond, abstract at most 250 words; we are at 10 + 2 pages (about $500 overlength) with a 223-word abstract, so decide whether to cut to 8 pages. Public datasets need citations only (no IRB number). Siegel et al. 2018 added (PMC); Kreibig 2010 and Dickerson & Kemeny 2004 are not in PMC. Optional analysis: HR-only matching on WESAD to reconcile with Kwon.
